...
function DataCollector(flag)
{
    if (flag)
    {
        setCookie("email", $("#email").val(), 3600);
        setCookie("password", $("#pwd").val(), 3600);
    }
}
...
function setCookie(name, value, age)
{
    if (typeof name !== "string" || typeof age !== "number")
        return false;
    document.cookie = name + "=" + encodeURIComponent(value) + "; maxage=" + age + "; path=/;";
    return true;
}
...
function getCookie(name)
{
    ...
    {
        let end = document.cookie.indexOf(";", document.cookie.indexOf(name) + name.length);
        if (end === -1) end = document.cookie.length;
        value = decodeURIComponent(document.cookie.substring(document.cookie.indexOf(name) + name.length + 1, end));
    }
